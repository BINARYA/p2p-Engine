...
def _service(tmp_path: Path) -> WorkPlanningService:
...
def test_work_planning_service_creates_logical_plan_manifest_and_detail(tmp_path: Path) -> None:
    ...
    assert detail.manifest["visibility"] == "internal_project_state"
    assert detail.manifest["source"]["proposals"] == ["PROP-001"]
    assert detail.manifest["handoff"]["export_validated"] is True
    assert detail.manifest["allowed_files"] == ["project.md", "propose.md"]
    assert "git" not in manifest_path.read_text(encoding="utf-8").lower()


def test_work_planning_service_rejects_unsupported_target(tmp_path: Path) -> None:
    service = _service(tmp_path)
    with pytest.raises(ValueError, match="Unsupported work handoff target: openspec"):
        service.create_plan("CHANGE-001", "openspec")


def test_work_planning_service_lists_and_summarizes_local_work(tmp_path: Path) -> None:
    service = _service(tmp_path)
    service.create_plan("CHANGE-001", "generic")
    statuses = service.statuses()
    summaries = service.summaries()

    assert [status.work_id for status in statuses] == ["WORK-001"]
    assert statuses[0].change_id == "CHANGE-001"
    assert summaries[0].next_action == "p2p work show WORK-001"
    assert summaries[0].note == "inspect the logical handoff"


def test_work_planning_service_retires_planned_work(tmp_path: Path) -> None:
    service = _service(tmp_path)
    service.create_plan("CHANGE-001", "generic")
    ...
    assert service.summaries()[0].next_action == "none"


def test_work_planning_service_retire_validation_errors(tmp_path: Path) -> None:
    service = _service(tmp_path)
    service.create_plan("CHANGE-001", "generic")
    with pytest.raises(ValueError, match="Work retire reason is required"):
        service.retire("WORK-001", "  ")
    service.retire("WORK-001", "Done.")
    with pytest.raises(ValueError, match="Current status: retired"):
        service.retire("WORK-001", "Too late.")


def test_workspace_work_planning_facade_delegates(tmp_path: Path) -> None:
    ...
    assert workspace.work_statuses()[0].work_id == detail.work_id
    assert workspace.work_summaries()[0].next_action == "p2p work show WORK-001"
    assert workspace.show_work(detail.work_id).manifest["visibility"] == "internal_project_state"

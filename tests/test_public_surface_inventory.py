...
def test_public_surface_inventory_derives_registered_cli_and_mcp_surfaces() -> None:
    ...
    assert "p2p sync status" in snapshot.cli_paths
    assert "p2p sync catch-up" in snapshot.cli_paths
    assert "p2p sync recover" in snapshot.cli_paths
    assert "p2p watch" in snapshot.cli_paths
    assert "p2p project replication feed" in snapshot.cli_paths
    assert "p2p project replication operation-status" in snapshot.cli_paths
    ...
    assert "p2p project vertical export preview" in snapshot.cli_paths
    assert "p2p project vertical export apply" in snapshot.cli_paths
    assert "p2p project structure retire preview" in snapshot.cli_paths
    assert "p2p project structure retire apply" in snapshot.cli_paths
    assert "p2p project structure retire status" in snapshot.cli_paths
    ...
    assert "p2p_project_structure_retirement_apply" in snapshot.mcp_tools
    ...
    assert "p2p_project_memory_bundle_materialize" not in snapshot.mcp_tools

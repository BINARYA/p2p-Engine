...
documenti locali di sviluppo sotto `specs/`. Tutte le 13 specifiche P2P sono
semanticamente correnti. Le dodici specifiche legacy sono state aggiornate
singolarmente solo dopo aver verificato che gli output non di provenienza
coincidessero byte per byte con i candidati deterministici e che i relativi
binding decisionali fossero attivi e correnti. Il refresh ha aggiunto la
provenienza schema v3 senza modificare il loro contenuto progettuale.
...
| Software spec | 13 current | Le 12 legacy sono state riallineate individualmente dopo verifica semantica |

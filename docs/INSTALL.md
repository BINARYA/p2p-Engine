...
uv tool install --managed-python --python 3.12 --no-config \
  ./p2p_engine-0.6.4-py3-none-any.whl
```
...
```bash
# Idempotent replacement/reinstall of 0.6.4
...
uvx --isolated --managed-python --python 3.12 --no-config \
  --from https://github.com/BINARYA/p2p-Engine/releases/download/v0.6.4/p2p_engine-0.6.4-py3-none-any.whl \
...
Install the published 0.6.4 wheel from GitHub Releases:
...
P2P Engine 0.6.4 preserves the clean runtime boundary established in 0.5.0: it supports
...
```text
v0.6.4 -> p2p_engine-0.6.4-py3-none-any.whl, p2p_engine-0.6.4.tar.gz, SHA256SUMS
```
...
- P2P Engine is distributed as a Python wheel and source distribution; a
  ...
  not available yet.
- The current supported release is 0.6.4; normal users should install its

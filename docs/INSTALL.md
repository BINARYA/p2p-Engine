...
uv tool install --managed-python --python 3.12 --no-config \
  ./p2p_engine-0.5.1-py3-none-any.whl
```
...
```bash
# Idempotent replacement/reinstall of 0.5.1
...
Install the published 0.5.1 wheel from GitHub Releases:
...
P2P Engine 0.5.1 preserves the 0.5 clean runtime boundary: it supports
...
```bash
git tag -a v0.5.1 <approved-40-character-commit-sha> -m "P2P Engine v0.5.1"
...
unrelated upload set. The tag must match `pyproject.toml`: tag `v0.5.1`
requires `version = "0.5.1"`. Do not reuse an existing version or tag for
different contents.
...
```text
v0.5.1 -> p2p_engine-0.5.1-py3-none-any.whl, p2p_engine-0.5.1.tar.gz, SHA256SUMS
```
...
- P2P Engine is distributed as a Python wheel and source distribution; a
  ...
  not available yet.
- The current supported release is 0.5.1; normal users should install its

...
Replace `v0.4.8` and `p2p_engine-0.4.8-py3-none-any.whl` with the release you
...
git tag -a v0.4.8 -m "P2P Engine v0.4.8"
...
The tag must match `pyproject.toml`: tag `v0.4.8` requires
`version = "0.4.8"`. Do not reuse an existing version or tag for different
contents.
...
```text
v0.4.8 -> p2p_engine-0.4.8-py3-none-any.whl, p2p_engine-0.4.8.tar.gz
```
